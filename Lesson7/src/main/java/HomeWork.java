public class HomeWork {
    ...
    1) Создаем класс военкомат by.tms.model.MilitaryOffice
    2) Создать в классе by.tms.model.MilitaryOffice конструктор, который принимает by.tms.model.PersonRegistry
    3) by.tms.model.PersonRegistry содержит список людей (by.tms.model.Person), которые могут быть призваны в армию
    4) Создать класс by.tms.model.Person, который будет содержать следующие поля имя, возраст, пол, адрес(страна, город)
    Военкомат должен уметь:
    a) вывести имена всех людей годных к военной службе на текущий момент времени, которые есть в by.tms.model.PersonRegistry
    T.е нужно написать метод в классе by.tms.model.MilitaryOffice, который будет отфильтровывать годных к службе призывников
    ...
    5) Создать класс Main, наполнить by.tms.model.PersonRegistry людьми, по желанию можно создавать людей через консоль
    */
